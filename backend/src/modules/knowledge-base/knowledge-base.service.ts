...
export class KnowledgeBaseService {
  ...
  async onModuleInit() {
    await this.qdrantService.waitForInit();
    await this.qdrantService.createCollection(this.collectionName);
  }
  ...
  async search(
    ...
    try {
      ...
      const qdrantResults = await this.qdrantService.searchWithPayloadFilter(
        this.collectionName,
      ...
      this.logger.warn(
        `Qdrant vector search failed/disabled, falling back to PostgreSQL chunk search: ${e.message}`,
      );
    }

    // Fallback to PostgreSQL database chunk search if Qdrant returned no results
    if (results.length === 0) {
      const dbChunks = await this.db.queryMany<KBChunk>(
        `SELECT * FROM kb_chunks WHERE knowledge_base_id = $1 AND content ILIKE $2 LIMIT $3`,
        [knowledgeBaseId, `%${query}%`, limit],
      );

      results = dbChunks.map((chunk) => ({
        chunkId: chunk.id,
        content: chunk.content,
        score: 0.85,
        documentId: chunk.documentId || null,

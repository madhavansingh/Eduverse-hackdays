...
export class ExamCloneController {
  ...
  @Post(':id/reanalyze')
  @ApiOperation({ summary: 'Re-analyze exam clone' })
  @ApiResponse({ status: 200, description: 'Exam clone re-analyzed' })
  async reanalyze(@CurrentUser() user: JwtPayload, @Param('id') id: string) {
    await this.examCloneService.reanalyze(id, user.sub);
    return { message: 'Re-analysis initiated successfully' };
  }

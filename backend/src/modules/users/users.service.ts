...
export class UsersService {
  ...
  async create(dto: CreateUserDto): Promise<User> {
    ...
    const createdUser = this.mapUser(result);
    this.logger.log(`User created in PostgreSQL: ${createdUser.id}`);
    return createdUser;
  }
  ...
  async findByEmail(email: string): Promise<User | null> {
    ...
    const result = await this.db.queryOne<User>('SELECT * FROM users WHERE email = $1', [targetEmail]);
    return result ? this.mapUser(result) : null;
  }

  async findByGoogleId(googleId: string): Promise<User | null> {
    const result = await this.db.queryOne<User>('SELECT * FROM users WHERE google_id = $1', [googleId]);
    return result ? this.mapUser(result) : null;
  }
  ...
  async update(id: string, dto: UpdateUserDto): Promise<User> {
    ...
    const updatedUser: User = {
      ...
      educationLevel: dto.educationLevel !== undefined ? dto.educationLevel : user.educationLevel,
      subjects: dto.subjects !== undefined ? dto.subjects : user.subjects,
      profileCompleted: dto.profileCompleted !== undefined ? dto.profileCompleted : user.profileCompleted,
      preferences: dto.preferences !== undefined ? dto.preferences : user.preferences,
      updatedAt: new Date(),
    };
    ...
    try {
      ...
      this.logger.error(`Database update failed (${dbErr.message}) for ${id}`);
    }

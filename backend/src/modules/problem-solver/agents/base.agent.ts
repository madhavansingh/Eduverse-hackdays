...
import { AiService, ChatMessage, extractAndRepairJson } from '../../ai/ai.service';
...
export abstract class BaseAgent {
  ...
  async execute(context: AgentContext): Promise<AgentResult> {
    ...
    const response = await this.aiService.completeJson<{
      ...
      reasoning: string;
      metadata?: Record<string, unknown>;
    }>(messages, { model: 'gemini-2.5-pro', maxTokens: 4096 });
  ...
  async *executeStream(
    ...
    for await (const chunk of this.aiService.streamComplete(messages, {
    ...
    try {
      // Gemini sometimes wraps streamed JSON in a ```json fence. The regular
      // JSON parser then fails and causes the whole transport payload to be
      // saved and rendered as the solution. Use the same recovery parser as
      // non-streaming requests so the persisted result is always clean text.
      ...
      if (typeof parsed.output !== 'string' || !parsed.output.trim()) {
        throw new Error('AI response did not include a solution output');
      }
      yield {
        type: 'result',
        data: {

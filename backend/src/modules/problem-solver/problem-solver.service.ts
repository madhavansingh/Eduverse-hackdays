...
export class ProblemSolverService {
  ...
  private resolveFinalAnswer(
    verifierResult: AgentResult,
    solverResult: AgentResult,
  ): string | null {
    const metadataAnswer = [verifierResult.metadata?.finalAnswer, solverResult.metadata?.finalAnswer]
      .find((value): value is string => typeof value === 'string' && value.trim().length > 0 && !/^see solution$/i.test(value.trim()));
    if (metadataAnswer) return metadataAnswer.trim();

    const labeledAnswer = solverResult.output.match(
      /(?:\*\*)?final answer(?:\*\*)?\s*:?\s*([^\n]+)/i,
    )?.[1];
    return labeledAnswer?.trim() || null;
  }
...
Return ONLY the extracted text exactly as shown in the image, preserving the original language and characters.`;
  ...
  async solve(sessionId: string, userId: string): Promise<ProblemSolvingSession> {
    ...
    try {
      ...
      const finalAnswer = this.resolveFinalAnswer(verifierResult, solverResult);

...
function WelcomeVisual() {
  ...
  return (
    <div className="relative w-48 h-48 mx-auto mb-4">
      ...
      >
        <img src="/logos/eduverse-logo.png" alt="Eduverse" className="w-20 h-20 object-contain drop-shadow-2xl" />
      </motion.div>
      {icons.map((Icon, i) => {
...
// Map slide key to visual component
function SlideVisual({ visualKey }: { visualKey: string }) {
  switch (visualKey) {
    case 'welcome': return <WelcomeVisual />;
    case 'studySets': return <StudySetsVisual />;
    case 'aiChat': return <AiChatVisual />;
    case 'problemSolver': return <ProblemSolverVisual />;
    case 'examClone': return <ExamCloneVisual />;
    case 'liveQuiz': return <LiveQuizVisual />;
    case 'spacedRepetition': return <SpacedRepVisual />;
    case 'analytics': return <AnalyticsVisual />;
    case 'handwritingOcr': return <HandwritingVisual />;
    case 'adFree': return <AdFreeVisual />;
    default: return null;
  }
}
    ...
    <div className={`min-h-screen flex flex-col bg-gradient-to-br ${slide.bg} ${slide.bgDark} transition-colors duration-700`}>
      ...
      <div className="flex items-center justify-between px-6 py-4 relative z-10">
        <div className="flex items-center gap-2">
          <img src="/logos/eduverse-logo.png" alt="Eduverse" className="w-11 h-11 object-contain" />
          <span className="text-xl font-bold bg-gradient-to-r from-blue-600 via-cyan-500 to-purple-600 bg-clip-text text-transparent">Eduverse</span>
        </div>
...
function SetupWizard() {
  ...
  return (
    <div className="min-h-screen h-screen bg-gradient-to-br from-green-50/50 via-emerald-50/30 to-teal-50/40 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 flex flex-col justify-between overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-8 py-4 shrink-0">
        <div className="flex items-center gap-2.5">
          <img src="/logos/eduverse-logo.png" alt="Eduverse" className="w-10 h-10 object-contain" />
          <span className="text-xl font-bold bg-gradient-to-r from-blue-600 via-cyan-500 to-purple-600 bg-clip-text text-transparent">Eduverse</span>
        </div>
        {step !== 'done' && (
          <button
      ...
      {/* Content */}
      <div className="flex-1 flex items-center justify-center px-6 py-4 overflow-y-auto min-h-0">
          ...
          <AnimatePresence mode="wait" custom={direction}>
            ...
            >
              ...
              {step === 'education' && (
                <div>
                  <div className="text-center mb-6">
                    ...
                    <h2 className="text-2xl md:text-3xl font-black mb-1.5">{t('onboarding.education.title')}</h2>
                    <p className="text-sm text-muted-foreground">{t('onboarding.education.description')}</p>
                  </div>
              ...
              {step === 'subjects' && (
                <div>
                  <div className="text-center mb-6">
                    <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-purple-500 to-violet-500 flex items-center justify-center mx-auto mb-3 shadow-lg shadow-purple-500/20">
                      <BookOpen className="w-6 h-6 text-white" />
                    </div>
                    <h2 className="text-2xl md:text-3xl font-black mb-1.5">{t('onboarding.subjects.title')}</h2>
                    <p className="text-sm text-muted-foreground">{t('onboarding.subjects.description')}</p>
                  </div>
                  ...
                  {selectedSubjects.filter((s) => !SUBJECTS.includes(s)).length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-4">
                      {selectedSubjects.filter((s) => !SUBJECTS.includes(s)).map((subject) => (
                        <span key={subject} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-500 text-white rounded-full text-sm font-semibold">
                          {subject}
                          <button onClick={() => toggleSubject(subject)}><X className="w-3.5 h-3.5" /></button>
                        </span>
                      ))}
                    </div>
                  )}
                  <div className="flex gap-2.5">
                    ...
                    />
                    <Button variant="outline" size="default" onClick={addCustomSubject} disabled={!customSubject.trim()} className="font-bold">
              ...
              {step === 'goal' && (
                <div>
                  <div className="text-center mb-5">
                    ...
                    <h2 className="text-2xl md:text-3xl font-black mb-1">{t('onboarding.goal.title')}</h2>
                    <p className="text-sm text-muted-foreground">{t('onboarding.goal.description')}</p>
                  </div>
                  <div className="space-y-2.5">
                    {GOALS.map((g) => (
                      <button
                        ...
                        className={cn(
                          'w-full p-3.5 md:p-4 rounded-2xl border-2 text-left transition-all flex items-center gap-3.5',
                          goal === g.id
                            ? 'border-green-500 bg-green-500/5 shadow-md shadow-green-500/10'
                            : 'border-border hover:border-muted-foreground/30 bg-card hover:shadow-xs'
                        )}
                      >
                        <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center shrink-0', goal === g.id ? 'bg-green-500/10' : 'bg-muted')}>
                          <g.icon className={cn('w-5 h-5', goal === g.id ? 'text-green-500' : 'text-muted-foreground')} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-bold leading-snug">{g.label}</p>
                          <p className="text-xs text-muted-foreground leading-snug truncate">{g.desc}</p>
                        </div>
                        {goal === g.id && (
              ...
              {step === 'done' && (
                <div className="text-center">
                  ...
                  <motion.h2 initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="text-3xl md:text-4xl font-black mb-2.5">
                    {t('onboarding.done.title')}
                  </motion.h2>
                  <motion.p initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="text-sm text-muted-foreground mb-6 max-w-md mx-auto">
                    {t('onboarding.done.description')}
                  </motion.p>
                  <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="grid grid-cols-3 gap-3 mb-6 max-w-lg mx-auto">
                    {[
                      ...
                      <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 + i * 0.1 }}
                        className="p-4 bg-card rounded-2xl border-2 border-border hover:border-green-300 dark:hover:border-green-700 transition-all"
                      >
                        <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${f.color} flex items-center justify-center mx-auto mb-2.5 shadow-md ${f.shadow}`}>
                  ...
                  <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 }}>
                    <Button size="lg" onClick={() => navigate('/dashboard')}
                      className="px-10 h-12 text-base font-bold bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 shadow-lg shadow-green-500/25"
                    >
      ...
      {/* Footer Navigation */}
      {step !== 'done' && (
        <div className="px-8 py-4 border-t border-border/40 bg-background/50 backdrop-blur-xs shrink-0">
          <div className="max-w-2xl mx-auto flex items-center justify-between">
            <Button variant="ghost" size="default" onClick={goBack} disabled={step === 'education'} className="font-semibold text-sm">
              <ArrowLeft className="w-4 h-4 mr-2" />
              {t('onboarding.back')}
            </Button>
            {step === 'goal' ? (
              <Button size="lg" onClick={handleComplete} disabled={!canProceed() || saving}
                className="px-8 h-11 text-sm font-bold bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 shadow-md shadow-green-500/25"
              >

...
export default function ExamClonePage() {
  ...
  const handleRetryAnalysis = async (id: string) => {
    setRetryingId(id);
    try {
      await api.post(ENDPOINTS.examClone.reanalyze(id));
      setExams((prev) =>
  ...
  // Auto-poll exam list when any exam is processing
  useEffect(() => {
    const hasProcessing = exams.some((e) => e.status === 'pending' || e.status === 'processing');
    if (hasProcessing) {
      if (!statusPollRef.current) {
        statusPollRef.current = setInterval(async () => {
          const updated = await fetchExams(true);
          const stillProcessing = updated.some((e) => e.status === 'pending' || e.status === 'processing');
          if (!stillProcessing && statusPollRef.current) {
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {exam.status === 'failed' && (
                            <Button
                              ...
                              onClick={() => handleRetryAnalysis(exam.id)}
                            >
                              {retryingId === exam.id ? (
                                <>

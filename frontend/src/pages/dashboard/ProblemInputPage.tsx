...
export function ProblemInputPage() {
  ...
  const handleSolve = async () => {
    ...
    let finalProblem = problemText.trim();
    ...
    if (uploadedImage) {
      ...
      try {
        ...
        const res = await api.post(ENDPOINTS.problemSolver.ocr, formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
        const ocrText = res.data?.text?.trim();
        if (ocrText) {
          finalProblem = finalProblem ? `${finalProblem}\n\n${ocrText}` : ocrText;
        }
      } catch (err) {
        console.error('OCR Extraction failed:', err);
        if (!finalProblem) {
          useProblemSolverStore.setState({ error: 'Failed to extract text from image. Please enter problem manually.' });
    ...
    const sessionId = await createAndSolve(finalProblem, subjectVal);
    if (sessionId) {
    ...
    <DashboardLayout>
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <Button
            onClick={handleSolve}
            disabled={(!problemText.trim() && !uploadedImage) || isLoading || isOcrProcessing}
          ...
          >
            {isLoading || isOcrProcessing ? (
              <>
                ...
                {isOcrProcessing ? 'Extracting text from image...' : t('problemInput.creatingSession')}
              </>
            ) : (
              <>

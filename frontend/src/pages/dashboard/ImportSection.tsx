...
// ─── Feature Flags ───────────────────────────────────────────────────
// Set to false to re-enable all import methods after the hackathon demo.
...
const ALL_IMPORT_SOURCES = [
  ...
  // ── Hidden in HACKATHON_MODE — re-enable by setting HACKATHON_MODE = false ──
  { key: 'youtube' as const, labelKey: 'importSection.youtubeVideo', descKey: 'importSection.youtubeVideoDesc', icon: Video, color: 'bg-red-500/10 text-red-500', borderColor: 'hover:border-red-500/40', hackathonHidden: true },
  { key: 'audio-record' as const, labelKey: 'importSection.recordAudio', descKey: 'importSection.recordAudioDesc', icon: Mic, color: 'bg-emerald-500/10 text-emerald-500', borderColor: 'hover:border-emerald-500/40', hackathonHidden: true },
  { key: 'audio-file' as const, labelKey: 'importSection.audioFile', descKey: 'importSection.audioFileDesc', icon: Music, color: 'bg-blue-500/10 text-blue-500', borderColor: 'hover:border-blue-500/40', hackathonHidden: true },
  { key: 'website' as const, labelKey: 'importSection.websiteUrl', descKey: 'importSection.websiteUrlDesc', icon: Globe, color: 'bg-purple-500/10 text-purple-500', borderColor: 'hover:border-purple-500/40', hackathonHidden: true },
  { key: 'camera' as const, labelKey: 'importSection.cameraScan', descKey: 'importSection.cameraScanDesc', icon: Camera, color: 'bg-amber-500/10 text-amber-500', borderColor: 'hover:border-amber-500/40', hackathonHidden: true },
  { key: 'google-docs' as const, labelKey: 'importSection.googleDocs', descKey: 'importSection.googleDocsDesc', icon: FileSpreadsheet, color: 'bg-sky-500/10 text-sky-500', borderColor: 'hover:border-sky-500/40', hackathonHidden: true },
  { key: 'handwriting' as const, labelKey: 'importSection.handwrittenNotes', descKey: 'importSection.handwrittenNotesDesc', icon: PenLine, color: 'bg-indigo-500/10 text-indigo-500', borderColor: 'hover:border-indigo-500/40', hackathonHidden: true },
];

const IMPORT_SOURCES = HACKATHON_MODE
  ? ALL_IMPORT_SOURCES.filter((s) => !('hackathonHidden' in s && s.hackathonHidden))
  : ALL_IMPORT_SOURCES;
...
export function ImportSection({ onCardsImported, studySetId }: ImportSectionProps) {
  ...
  const processWithAI = async (
    ...
    try {
      ...
      // Axios' generic "Request failed with status code ..." hides the
      // useful backend message (for example, a temporary AI quota limit).
      const msg = extractErrorMessage(err, 'Processing failed. Please try again.');
  ...
  return (
    <AnimatePresence mode="wait">
          ...
          <div className={`grid gap-3 ${
            ...
            {IMPORT_SOURCES.map((source) => (
              <button
                ...
                className={`flex flex-col items-center gap-2.5 p-5 bg-card border border-border rounded-xl text-center transition-all hover:shadow-md ${source.borderColor}`}
              >
                <div className={`w-12 h-12 rounded-xl ${source.color} flex items-center justify-center`}>
                  <source.icon className="w-6 h-6" />
                </div>
                <div>
                  <p className="text-sm font-semibold">{t(source.labelKey)}</p>
                  <p className="text-[11px] text-muted-foreground mt-0.5 leading-snug">{t(source.descKey)}</p>
                </div>
              </button>
            ))}
          </div>
      ...
      {/* ── Source Screens (hidden in HACKATHON_MODE) ── */}
      {!HACKATHON_MODE && step === 'youtube' && <YouTubeScreen key="youtube" onBack={goBack} onSubmit={handleYouTubeSubmit} />}
      {!HACKATHON_MODE && step === 'audio-record' && <AudioRecordScreen key="audio-record" onBack={goBack} onSubmit={handleAudioRecordSubmit} />}
      {!HACKATHON_MODE && step === 'audio-file' && <AudioFileScreen key="audio-file" onBack={goBack} onSubmit={handleAudioFileSubmit} />}
      {!HACKATHON_MODE && step === 'website' && <WebsiteScreen key="website" onBack={goBack} onSubmit={handleWebsiteSubmit} />}
      {!HACKATHON_MODE && step === 'camera' && <CameraScreen key="camera" onBack={goBack} onSubmit={handleCameraSubmit} />}
      {!HACKATHON_MODE && step === 'google-docs' && <GoogleDocsScreen key="google-docs" onBack={goBack} />}
      {!HACKATHON_MODE && step === 'handwriting' && <HandwritingScreen key="handwriting" onBack={goBack} onSubmit={handleHandwritingSubmit} />}

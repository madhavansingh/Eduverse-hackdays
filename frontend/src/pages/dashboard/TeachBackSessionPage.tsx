...
export function TeachBackSessionPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setActionError(extractErrorMessage(error, 'Could not submit your explanation. Please try again.'));
    }
    setSubmitting(false);
  };
  ...
  const handleEvaluate = async () => {
    ...
    try {
      ...
      setActionError(extractErrorMessage(error, 'Could not evaluate your explanation. Please try again.'));
    }
    setEvaluating(false);
  };
        ...
        {session.status === 'submitted' && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            <div className="bg-card border border-border rounded-2xl p-6">
              ...
              {actionError && (
                <p role="alert" className="mt-3 rounded-lg bg-red-500/10 px-3 py-2 text-sm text-red-600">
                  {actionError}
                </p>
              )}
            </div>

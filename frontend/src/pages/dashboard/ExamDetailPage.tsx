...
export default function ExamDetailPage() {
  ...
  const handleReanalyze = async () => {
    ...
    const confirmed = window.confirm(
      'Re-analyze will delete existing questions and re-extract them from the original PDF with improved AI. This will add specific data (formulas, graphs, values) to each question. Continue?'
    );
    if (!confirmed) return;
    try {
      setIsReanalyzing(true);
      await api.post(`/exam-clones/${id}/reanalyze`);
      // Poll for completion
      let attempts = 0;
      const poll = setInterval(async () => {
        attempts++;
        try {
          const res = await api.get<ExamClone>(ENDPOINTS.examClone.get(id));
          if (res.data.status === 'completed' || res.data.status === 'failed' || attempts > 60) {
            clearInterval(poll);
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <Button
            ...
            title="Re-extract questions from original PDF with improved AI (adds specific data like formulas and graphs)"
          >
            {isReanalyzing ? (

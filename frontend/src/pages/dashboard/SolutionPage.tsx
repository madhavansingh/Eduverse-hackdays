...
function parseAiJson(raw: string): unknown | null {
  ...
  for (const candidate of candidates) {
    try {
      ...
      // Try the next usable JSON candidate.
    }
  }
  return null;
}

function extractOutputFromMalformedAiJson(raw: string): string | null {
  // This keeps previously saved, fenced responses readable even if an older
  // response was truncated before the closing JSON object.
...
function deepExtractText(raw: unknown): string {
  ...
  return extractOutputFromMalformedAiJson(raw) || raw.replace(/^```(?:json)?\s*/i, '');
}
...
function extractMetadata(step: { output?: unknown; confidence?: number } | null): {
  ...
  const findMeta = (val: unknown): Record<string, unknown> | null => {
    ...
    if (typeof val === 'string') {
      ...
      return parsed === null ? null : findMeta(parsed);
    }
    return null;
  };
  ...
  const findConfidence = (val: unknown): number | null => {
    ...
    if (typeof val === 'string') {
      const parsed = parseAiJson(val);
      return parsed === null ? null : findConfidence(parsed);
    }
    return null;
  };
...
function SolutionContent({ text }: { text: unknown }) {
  const content = normalizeLatex(extractAgentOutput(text)).trim();
  ...
  return (
    <ReactMarkdown
      ...
      components={{
        h1: ({ children }) => <h2 className="text-lg font-semibold mt-5 mb-2 first:mt-0">{children}</h2>,
        ...
        ul: ({ children }) => <ul className="list-disc space-y-1 pl-5 mb-3">{children}</ul>,
        ol: ({ children }) => <ol className="list-decimal space-y-1 pl-5 mb-3">{children}</ol>,
        ...
        pre: ({ children }) => <pre className="my-3 overflow-x-auto rounded-lg bg-zinc-950 p-3 text-zinc-100">{children}</pre>,
      }}
    >
      {content}
    </ReactMarkdown>
...
function getFinalAnswerFromSolution(output: string): string | null {
  const match = output.match(/(?:\*\*)?final answer(?:\*\*)?\s*:?\s*([^\n]+)/i);
...
export function SolutionPage() {
  ...
  const loadAlternativeMethods = useCallback(async () => {
    ...
    setAltLoading(true);
    try {
      const methods = await problemSolverService.getAlternativeMethods(id);
      setAltMethods(
        methods.filter((method) => extractAgentOutput(method.solutionSteps?.output).trim().length > 20),
      );
    } catch (err) {
  ...
  const savedFinalAnswer = session?.finalAnswer?.trim();
  const realFinalAnswer =
    verifierMeta.finalAnswer ||
    solverMeta.finalAnswer ||
    (savedFinalAnswer && !/^see solution$/i.test(savedFinalAnswer) ? savedFinalAnswer : null) ||
    getFinalAnswerFromSolution(solutionOutput);
            ...
            <div className="space-y-3">
              ...
              {analysisOutput && (
                <div className="bg-blue-500/5 border border-blue-500/20 rounded-xl p-4 mb-4">
                  ...
                  <div className="text-sm text-muted-foreground">
                    <SolutionContent text={analysisOutput} />
                  </div>
                </div>
              )}
                  ...
                  {expandedSteps.has(idx) && (
                    <div className="px-4 pb-4 border-t border-border">
                      <div className="pt-3 text-sm leading-relaxed">
                        <SolutionContent text={step.content} />
                      </div>
              ...
              {verificationOutput && (
                <div className="bg-green-500/5 border border-green-500/20 rounded-xl p-4 mt-4">
                  ...
                  <div className="text-sm text-muted-foreground">
                    <SolutionContent text={verificationOutput} />
                  </div>
                </div>
              )}
              ...
              {altMethods.map((method, idx) => (
                ...
                >
                  ...
                  <div className="text-sm bg-muted/30 rounded-lg p-3">
                    <SolutionContent text={method.solutionSteps?.output} />
                  </div>
                </motion.div>
              ))}
            </div>
